...
{
    // Funcion para llamar a todos los Usuarios del sistema -- se usa la tabla de usuarios en vez de la de user por ahora
    ...
    // Funcion para alamacenar los Usuarios dentro de la base de datos --  se usa la tabla de usuarios en vez de la de user por ahora
    public function store(Request $request){
        ...
        if($validator->fails()) {
            $data = [
                'mensaje' => 'Error en la validacion, datos incorrectos',
    ...
    public function show($id){
        ...
        if (!$usuario){
            $data = [
                'mensaje' => 'No se encontro al Usuario para editar',
                'status' => 404
            ];
            return response()->json($data, 404);
        }

        // si el usuario fue encontrado lo colocara dentor de esta variable
        $data = [
            'usuario' => $usuario,
            'status' => 200
        ];
    ...
    public function update( Request $request, $id) {
        ...
        // // Aqui tomamos los nuevos datos que nos hallan mandado y los colocamos en donde corresponde
        // $usuario->id_usuario = $request->documento;
        // $usuario->cod_rol = $request->rol;
        // $usuario->nom_usuario = $request->nombre;
        // $usuario->ape_usuario = $request->apellido;
        // $usuario->email_usuario = $request->email;
        // $usuario->tele_usuario = $request->telefono;
        // $usuario->cont_usuario = $request->password;
